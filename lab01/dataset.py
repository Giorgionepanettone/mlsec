...
class DatasetInterface:
    ...
    def __init__(self):
        ...
        self.data = {
            "train": {"x": x_train, "y": y_train, "y_multi": y_train_multi},
    ...
    def info(self):
        return (
            f"\tTraining samples: {self.data['train']['x'].shape[0]};\n"
...
def load_dataset():
    ...
    df_train[["protocol_type", "service", "flag", "attack"]] = pd.DataFrame(
        categs_encoded, columns=categs_feats.columns
    )

    categs_feats = df_test[["protocol_type", "service", "flag", "attack"]]
    encoder = OrdinalEncoder()
    categs_encoded = encoder.fit_transform(categs_feats)
    df_test[["protocol_type", "service", "flag", "attack"]] = pd.DataFrame(
        categs_encoded, columns=categs_feats.columns
    )
    ...
    df_train = df_train.drop(val_indices, axis = 0)
    ...
    val_features = df_val.drop(columns=["attack", "label"])
    val_labels = df_val[["attack", "label"]]
    ...
    y_train_multi = train_labels["attack"].to_numpy()

    y_val = val_labels["label"].to_numpy()
    y_val_multi = val_labels["attack"].to_numpy()
